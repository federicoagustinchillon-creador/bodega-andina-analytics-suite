...
RAW_DIR = os.path.join(BASE_DIR, "01_Limpieza_de_Datos", "raw")

def update_fact_operaciones_planta():
    print("--- Actualizando ProduccionPlanta.csv ---")
    p = os.path.join(GOLD_DIR, "ProduccionPlanta.csv")
    ...
    print(f"ProduccionPlanta.csv actualizado con {len(df)} registros y columnas Latitud/Longitud.")
    return df

def create_fact_inventario_guarda():
    print("\n--- Creando InventarioGuarda.csv ---")
    p = os.path.join(GOLD_DIR, "InventarioGuarda.csv")
    ...
    print(f"InventarioGuarda.csv creado exitosamente con {len(df_inv)} registros (12 meses x 5 SKUs x 4 vasijas).")
...
def update_dim_cuentas_contables():
    print("\n--- Actualizando PlanCuentas.csv ---")
    p = os.path.join(GOLD_DIR, "PlanCuentas.csv")
    ...
    print(f"PlanCuentas.csv actualizado con {len(df_cta)} cuentas.")
...
def update_fact_opex_mensual():
    print("\n--- Actualizando GastosOperativos.csv con cuentas industriales ---")
    p = os.path.join(GOLD_DIR, "GastosOperativos.csv")
    ...
    print(f"GastosOperativos.csv guardado con {len(df_final_opx)} registros (cuadratura exacta comprobada).")
    return df_final_opx

# 03_Inteligencia_Comercial · Bodega & Agroindustria Andina S.A.
...
**Capa de Datos:** `01_Limpieza_de_Datos/curated_gold`  
...
El proyecto **03_Inteligencia_Comercial** proporciona a la Dirección Comercial, Gerencia de Ventas y al Controller Comercial una herramienta analítica avanzada para evaluar:
...
|-- Inteligencia_Comercial.pbip
|-- Inteligencia_Comercial.SemanticModel/
|   |-- .platform
|   |-- definition.pbism
|   \-- definition/
...
|           |-- Calendario.tmdl
|           |-- CentrosCosto.tmdl
|           |-- Productos.tmdl (SKU, Descripción, Línea, CategoriaABC)
|           |-- Ventas.tmdl (Ventas granulares SAP SD)
|           |-- PresupuestoVentas.tmdl (Metas comerciales unpivoteadas)
|           \-- _Medidas_Comercial.tmdl (40+ medidas DAX dinámicas)
\-- Inteligencia_Comercial.Report/
    |-- .platform
    |-- definition.pbir
    |-- StaticResources/
    |   \-- RegisteredResources/
...
- `Ventas[DateKey]` -> `Calendario[DateKey]` (N:1)
- `Ventas[ProductoID]` -> `Productos[ProductoID]` (N:1)
- `PresupuestoVentas[DateKey]` -> `Calendario[DateKey]` (N:1)
- `PresupuestoVentas[ProductoID]` -> `Productos[ProductoID]` (N:1)
- `PresupuestoVentas[CentroCostoID]` -> `CentrosCosto[CentroCostoID]` (N:1)

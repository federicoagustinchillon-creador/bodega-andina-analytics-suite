...
`01_Limpieza_de_Datos/curated_gold` (ruta relativa a la raíz del repo)

### 2.1 Tablas del Modelo
- `Calendario`: Calendario corporativo anual con granularidad diaria, jerarquías de mes, trimestre, semestre y marcas de día hábil.
- `Productos`: Catálogo maestro de referencias (SKU, Línea, Costo Estándar Unitario, Precio Presupuestado).
- `CentrosCosto`: Estructura organizativa de centros de responsabilidad (CC-101 Elaboración y Molienda, CC-102 Fraccionamiento y Embalaje, CC-103 Mantenimiento y Servicios, CC-104 Logística y Despacho).
- `PlanCuentas`: Plan maestro contable segmentado en COGS (Materia Prima Uva, Insumos Secos, Mano de Obra Directa) y OPEX (Fletes, Sueldos, Energía Eléctrica T3, Comisiones).
- `ProduccionPlanta`: Registro transaccional de 350 remitos de ingreso de uva durante la vendimia 2025 (Kilos Entrada, Litros Obtenidos, Rendimiento %, Grados Brix, Finca de Origen, Variedad, Calificación Sanitaria, Latitud, Longitud).
- `InventarioGuarda`: Detalle mensual de crianza y guarda por ProductoID y TipoVasija (Piletas de Acero Inoxidable, Barricas de Roble Frances, Barricas de Roble Americano, Botellero en Estiba Climatizada) con Litros, ValorInventario, MermaPct, DIODias y EstadoCalidad.
- `GastosOperativos`: Ejecución mensual presupuestaria por centro de costo y cuenta contable con desgloses industriales fabriles (CTA-201, CTA-202, CTA-203, CTA-204) para CC-101, CC-102 y CC-103.
- `CapitalTrabajo`: Métricas mensuales de inventario, cuentas por cobrar, cuentas por pagar, DSO, DIO, DPO, CCC y Necesidad Operativa de Fondos (NOF).
- `_Medidas_Operaciones`: Tabla técnica dedicada para medidas DAX dinámicas.

### 2.2 Diagrama de Relaciones
- `ProduccionPlanta[DateKey]` $\rightarrow$ `Calendario[DateKey]` (N:1)
- `ProduccionPlanta[CentroCostoID]` $\rightarrow$ `CentrosCosto[CentroCostoID]` (N:1)
- `InventarioGuarda[DateKey]` $\rightarrow$ `Calendario[DateKey]` (N:1)
- `InventarioGuarda[ProductoID]` $\rightarrow$ `Productos[ProductoID]` (N:1)
- `GastosOperativos[DateKey]` $\rightarrow$ `Calendario[DateKey]` (N:1)
- `GastosOperativos[CentroCostoID]` $\rightarrow$ `CentrosCosto[CentroCostoID]` (N:1)
- `GastosOperativos[CuentaID]` $\rightarrow$ `PlanCuentas[CuentaID]` (N:1)
- `CapitalTrabajo[DateKey]` $\rightarrow$ `Calendario[DateKey]` (N:1)
...
1. **Total Kilos Uva:**
   ```dax
   Total Kilos Uva = SUM(ProduccionPlanta[KilosEntrada])
   ```
   Volumen agregado de materia prima ingresada a molienda (5.452.927,0 kg).

2. **Total Litros Mosto:**
   ```dax
   Total Litros Mosto = SUM(ProduccionPlanta[LitrosMostoObtenido])
   ```
...
4. **Grados Brix Ponderado:**
   ...
   DIVIDE(
       SUMX(ProduccionPlanta, ProduccionPlanta[GradosBrix] * ProduccionPlanta[KilosEntrada]), 
       [Total Kilos Uva], 
       0
   )
   ```
...
6. **Costo Total Fabril:**
   ...
   Costo Total Fabril = 
   CALCULATE(
       SUM(GastosOperativos[OPEXReal]), 
       GastosOperativos[CentroCostoID] IN {"CC-101", "CC-102", "CC-103"}
   )
   ```
...
7. **Costo Presupuesto Fabril:**
   ...
   CALCULATE(
       SUM(GastosOperativos[OPEXPresupuestado]), 
       GastosOperativos[CentroCostoID] IN {"CC-101", "CC-102", "CC-103"}
   )
   ```
...
12. **Valor Inventario Guarda:**
    ```dax
    Valor Inventario Guarda = AVERAGE(CapitalTrabajo[Inventario])
    ```
    Valorización contable promedio del stock inmovilizado ($207.959.619,22).

13. **Días de Inventario (DIO):**
    ```dax
    DIO Promedio Dias = AVERAGE(CapitalTrabajo[DIO_DiasInventario])
    ```
...
3. **Cero Hardcoding:** 100% de las medidas DAX se computan dinámicamente sobre las tablas `ProduccionPlanta`, `GastosOperativos`, `CapitalTrabajo` y dimensiones asociadas.
